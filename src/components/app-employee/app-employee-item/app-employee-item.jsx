...
export default class AppEmployeeItem extends Component {
  ...
  onIncrease = () => {
    ...
    const indexChange = dataMockup.findIndex((obj) => obj.id == this.state.id);
    dataMockup[indexChange].increase = this.state.increase
  }
  ...
  render() {
    ...
    const { increase, isFav } = this.state;

    let classes = "app-employee-item";
    let favClasses = "fav"

    if (increase) classes += " increased";
    if (isFav) favClasses += " activeFav"

    return (
      <div className={classes}>
        <h4 className="employee-name" onClick={this.toggleFav}>{name}</h4>
